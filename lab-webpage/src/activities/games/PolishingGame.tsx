...
const STAGES = [
  { label: "거친 연마 (#400)", description: "큰 스크래치를 제거합니다.", threshold: 40 },
  { label: "중간 연마 (#1200)", description: "표면을 평탄하게 만듭니다.", threshold: 40 },
  { label: "정밀 연마 (Diamond)", description: "거울처럼 매끄럽게 만듭니다.", threshold: 40 },
];
...
function PolishingGame() {
  ...
  const initCanvas = useCallback(() => {
    ...
    ctx.fillStyle = stage === 0 ? "#94a3b8" : stage === 1 ? "#cbd5e1" : "#e2e8f0";
  ...
  return (
    <div className="mx-[10%] my-12 font-sans select-none">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500">금속 시편을 문질러서 현미경 조직을 관찰해보세요!</p>
      </div>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100 gap-4">
            <div className="flex-1">
              <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">현재 공정</span>
              <h3 className="text-xl font-bold text-slate-800">{STAGES[stage].label}</h3>
            </div>
            <div className="flex-1 w-full max-w-xs">
              <div className="flex justify-between text-xs mb-1 font-bold">
                <span>공정 진행률</span>
                <span>{Math.round((progress / STAGES[stage].threshold) * 100)}%</span>
              </div>
              <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden">
                <div 
                  className="bg-blue-600 h-full transition-all duration-300"
                  style={{ width: `${Math.min((progress / STAGES[stage].threshold) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
          </div>
            ...
            {isFinished && (
              <div className="absolute inset-0 z-20 bg-black/70 backdrop-blur-sm flex items-center justify-center animate-[appear_0.5s_ease-out]">
                <div className="text-center p-8 bg-white rounded-2xl shadow-2xl max-w-sm mx-4">
                  ...
                  <h3 className="text-2xl font-bold text-title mb-2">연마 성공!</h3>
                  <p className="text-gray-600 mb-6 text-sm">표면이 완벽하게 다듬어졌습니다. 이제 현미경으로 조직을 선명하게 관찰할 수 있습니다.</p>
                  <div className="flex flex-col gap-2">
                    <button 
                      onClick={() => { setStage(0); setIsFinished(false); setProgress(0); }}
